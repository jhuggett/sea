...
func (p *Population) Create() (uint, error) {
	err := db.Conn().Create(&p.Persistent).Error
	return p.Persistent.ID, err
}

func (p *Population) DemandsPerDay() (Demands, error) {
	return StandardDemands, nil
}
...
var StandardDemands = Demands{
	...
	// 	Need: 1.5 / float64(timeline.Day),
	// 	Want: 1 / float64(timeline.Day),
	// },
	// items.EdibleMeat: Demand{
	// 	Need: .5 / float64(timeline.Day),
	// 	Want: 2 / float64(timeline.Day),
	// },
	items.Food: Demand{
		Need: 2.0 / float64(timeline.Day),
	},

	items.Fuel: Demand{
		Need: 1.0,
		Want: 0.5 / float64(timeline.Day),
	},
...
// Ever x amount of time, do the demand thing
