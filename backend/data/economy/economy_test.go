...
func TestXxx(t *testing.T) {
	...
	// minting industry
	mintingIndustryModel := industry.Industry{
		Persistent: data.Industry{
			EconomyID:      economyModel.Persistent.ID,
			ShareOfWorkers: 0,
			Product:        string(items.PieceOfEight),
		},
	}
	_, err = mintingIndustryModel.Create()
	if err != nil {
		t.Error(err)
	}
	...
	err = economyModel.Hydrate(WithIndustries, WithPopulations, WithMarkets)
	if err != nil {
		t.Error(err)
	}
	...
	grainMarket, err := economyModel.Market(items.Grain)
	if err != nil {
		t.Error(err)
	}
	...
	for productName, marketData := range marketDataOverTime {
		...
		fmt.Println(graph)
		graph = asciigraph.Plot(marketData.workerShare, asciigraph.Width(maxGraphWidth), asciigraph.Height(maxGraphHeight), asciigraph.Caption("Worker Share"))
		fmt.Println(graph)
		graph = asciigraph.PlotMany(
			...
			asciigraph.Width(maxGraphWidth),
			asciigraph.Height(maxGraphHeight),
			asciigraph.SeriesLegends("Historical Demand Average", "Historical Supply Average"),
		)
		fmt.Println(graph)
	...
	graph := asciigraph.PlotMany(relativeValues, asciigraph.SeriesLegends(productNames...), asciigraph.SeriesColors(asciigraph.Red, asciigraph.Green, asciigraph.Blue), asciigraph.Width(maxGraphWidth), asciigraph.Height(maxGraphHeight))
	...
	graph = asciigraph.PlotMany(populationsOverTime, asciigraph.SeriesLegends(populationNames...), asciigraph.SeriesColors(asciigraph.Red, asciigraph.Green, asciigraph.Blue), asciigraph.Width(maxGraphWidth), asciigraph.Height(maxGraphHeight))

	fmt.Println(graph)

	for _, population := range economyModel.Persistent.Populations {
		slog.Info("Population", "population", population)
	}

	assert.Equal(t, 5020.0, grainMarket.Surplus)
}
