...
func Register(r RegisterReq) (RegisterResp, error) {
	slog.Info("Register")
	...
	for _, continentData := range worldMap.Persistent.Continents {
		...
		port := &port.Port{
			Persistent: data.Port{
				...
				ContinentID: continentModel.Persistent.ID,
		...
		if err != nil {
			return RegisterResp{}, fmt.Errorf("failed to add item: %w", err)
		}
	...
	if err != nil {
		return RegisterResp{}, fmt.Errorf("failed to save fleet: %w", err)
	}
	...
	if err != nil {
		return RegisterResp{}, fmt.Errorf("failed to create deed: %w", err)
	}
	...
	err = db.Conn().Create(&sessionData).Error
	if err != nil {
		return RegisterResp{}, err
	}
